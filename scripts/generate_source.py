...
def RunGenerators(api: str, registry: str, video_registry: str, directory: str, styleFile: str, targetFilter: str, caching: bool):
    ...
    cachePathVk = os.path.join(tempfile.gettempdir(), f'vkobject_{os.getpid()}vk')
    cachePathVideo = os.path.join(tempfile.gettempdir(), f'vkobject_{os.getpid()}video')
    cacheVkObjectData = None
    cacheVideoObjectData = None
    if caching:
        EnableCaching()
    ...
    for index, target in enumerate(targets, start=1):
        ...
        generator.xmlName = generators[target]['registry']
        ...
        if generators[target]['registry'] == 'vk':
            reg.loadFile(registry)

        elif generators[target]['registry'] == 'video':
            reg.loadFile(video_registry)
        else:
        ...
        cachePath = cachePathVk if generators[target]['registry'] == 'vk' else cachePathVideo
        cacheData = cacheVkObjectData if generators[target]['registry'] == 'vk' else cacheVideoObjectData

        if caching and not cacheData:
            if os.path.isfile(cachePath):
                file = open(cachePath, 'rb')
                cacheData = pickle.load(file)
                file.close()

        if caching and (cacheData) and ('regenerate' not in generators[target] or not generators[target]['regenerate']):
    ...
    if os.path.isfile(cachePathVk):
        os.remove(cachePathVk)
    if os.path.isfile(cachePathVideo):
        os.remove(cachePathVideo)
