...
EXPORT_FUNCTION VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL vkGetDeviceProcAddr(VkDevice device, const char* pName)
{
    ...
    def generate_implementation(self, output_format, video=False):
        ...
        for basetype in self.vk.baseTypes.values():
            ...
            self.write(f'void dump_{output_format}_{basetype.name}(const {basetype.name}& object, const ApiDumpSettings& settings, const char* type_name, const char *var_name, int indents, const void* address = nullptr)')
            self.write('{')
            self.write(f'    dump_{output_format}_start(settings, OutputConstruct::value, type_name, var_name, indents, address);')
            if basetype.name in self.return_types:
        ...
        for sys in sortedSystemTypes.values():
            ...
            self.write(f'    dump_{output_format}_end(settings, OutputConstruct::value, indents);')
            self.write('}')
            if not sys.name in POINTER_TYPES:
    ...
    settings.stream() << "</div>";''')
        ...
        if not video:
            if output_format == 'text':
                ...
                self.write('        return;')
                self.write('    }\n')
                self.write('    settings.formatNameType(indents, var_name, type_name);')
                ...
                for struct in [ x for x in self.vk.structs.values() if not x.union ]:
                    ...
                    if struct.sType is not None:
                        ...
                        self.write(f'            dump_{output_format}_start(settings, OutputConstruct::api_struct, "{struct.name}", var_name, indents);')
            ...
            for struct in [ x for x in self.vk.structs.values() if not x.union ]:
                ...
                if struct.sType is not None:
                    self.write(f'    case {struct.sType}:')
                    self.write(f'        if (base_struct.pNext == NULL) {{ dump_{output_format}_nullptr(settings, "pNext", "{struct.name}", indents); }}')
                    self.write(f'        else {{ dump_{output_format}_{struct.name}(*reinterpret_cast<const {struct.name}*>(object), settings, type_name, var_name, indents); }}')
                    self.write('        break;')
                if struct.protect:
                    self.write(f'#endif // {struct.protect}')
            self.write('')
