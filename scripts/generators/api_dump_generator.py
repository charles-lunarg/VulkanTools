    ...
    def generate_text_header(self, video=False):
        ...
        for command in [x for x in self.vk.commands.values() if x.name not in FUNCTION_IMPLEMENTATION_IGNORE_LIST]:
            if command.protect:
                self.write(f'#if defined({command.protect})')
            if command.returnType != 'void':
                self.write(f'void dump_text_{command.name}(ApiDumpInstance& dump_inst, {command.returnType} result, {commandParameterDeclarationText(command)});')
            else:
                self.write(f'void dump_text_{command.name}(ApiDumpInstance& dump_inst, {commandParameterDeclarationText(command)});')
            if command.protect:
                self.write(f'#endif // {command.protect}')
        self.write('\n')
                ...
                if typeInfo.pointerLevels == 0:
                    if member.name != 'pNext':
                        if member.name == 'apiVersion':
                            self.write(f'    dump_text_value<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, OutputApiVersionTEXT);  // AET')
                        else:
                            self.write(f'    dump_text_value<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_text_{typeInfo.id});  // AET')
                    else:
                    ...
                    if typeInfo.arrayLength is None:
                        self.write(f'    dump_text_pointer<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_text_{typeInfo.id});')

                    else:
                        if not typeInfo.lengthIsMember:
                            self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // AQA')

                        elif typeInfo.lengthIsMember and member.name != 'pCode':
                            if typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // BQA')

                            elif not typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                if typeInfo.arrayLength == 'rasterizationSamples':
                                    self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, (object.{typeInfo.arrayLength} + 31) / 32, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // BQB')
                                else:
                                    if typeInfo.maxArrayLength is not None:
                                        self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, std::min(object.{typeInfo.arrayLength}, {typeInfo.maxArrayLength}), settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // BQB')
                                    else:
                                        self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, object.{typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // BQB')
    ...
    if(settings.showAddress())
        ...
        settings.stream() << "address (Union):\\n";''')
            for member in union.members:
                ...
                if typeInfo.pointerLevels == 0:
                    self.write(f'    dump_text_value<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // LET')
                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'    dump_text_pointer<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_text_{typeInfo.id});')
                    else:
                        self.write(f'    dump_text_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_text_{typeInfo.id}); // GQA')
        ...
        if not video:
            ...
            for struct in [ x for x in self.vk.structs.values() if not x.union ]:
                ...
                if struct.sType is not None:
                    s = [ y for y in self.vk.enums['VkStructureType'].fields if y.name == struct.sType][0]
                    self.write(f'        case {s.value}:')
        ...
        for command in [x for x in self.vk.commands.values() if x.name not in FUNCTION_IMPLEMENTATION_IGNORE_LIST]:
            ...
            for param in command.params:
                ...
                if typeInfo.pointerLevels == 0:
                    self.write(f'        dump_text_value<const {typeInfo.baseType}>({param.name}, settings, "{typeInfo.type}", "{param.name}", 1, dump_text_{typeInfo.id}); // MET')
                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'        dump_text_pointer<const {typeInfo.baseType}>({param.name}, settings, "{typeInfo.type}", "{param.name}", 1, dump_text_{typeInfo.id});')
                    else:
                        self.write(f'        dump_text_array<const {typeInfo.baseType}>({param.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{param.name}", 1, dump_text_{typeInfo.id}); // HQA')
...
{{
        ...
        for struct in [ x for x in self.vk.structs.values() if not x.union ]:
            ...
            for member in struct.members:
                ...
                if typeInfo.pointerLevels == 0:
                    if member.name != 'pNext':
                        if member.name == 'apiVersion':
                            self.write(f'    dump_html_value<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, OutputApiVersionHTML);')
                        else:
                            self.write(f'    dump_html_value<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_html_{typeInfo.id});')
                    else:
                        self.write('    if(object.pNext != nullptr){')
                    ...
                    if typeInfo.arrayLength is None:
                        self.write(f'    dump_html_pointer<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", indents + 1, dump_html_{typeInfo.id});')

                    else:
                        if not typeInfo.lengthIsMember:
                            self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRR')

                        elif typeInfo.lengthIsMember and member.name != 'pCode':
                            if typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRS')

                            elif not typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                if typeInfo.arrayLength == 'rasterizationSamples':
                                    self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, (object.{typeInfo.arrayLength} + 31) / 32, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRT')
                                else:
                                    if typeInfo.maxArrayLength is not None:
                                        self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, std::min(object.{typeInfo.arrayLength}, {typeInfo.maxArrayLength}), settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // BQB')
                                    else:
                                        self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, object.{typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRT')

                self.write('')
                if struct.name == 'VkShaderModuleCreateInfo':
                    if member.name == 'pCode':
                        self.write('    if(settings.showShader())')
                        self.write(f'        dump_html_array<const {typeInfo.baseType}>(object.{member.name}, object.{typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRU')
                        self.write('    else')
    ...
    settings.stream() << "</div></summary>";''')
            for member in union.members:
                ...
                if typeInfo.pointerLevels == 0:
                    ...
                    if typeInfo.arrayLength is None:
                        ...
                        self.write(f'    dump_html_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", indents + 1, dump_html_{typeInfo.id}); // ZRY')
        ...
        for command in [x for x in self.vk.commands.values() if x.name not in FUNCTION_IMPLEMENTATION_IGNORE_LIST]:
            ...
            for param in command.params:
                ...
                if typeInfo.pointerLevels == 0:
                    self.write(f'        dump_html_value<const {typeInfo.baseType}>({param.name}, settings, "{typeInfo.type}", "{param.name}", 1, dump_html_{typeInfo.id});')

                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'        dump_html_pointer<const {typeInfo.baseType}>({param.name}, settings, "{typeInfo.type}", "{param.name}", 1, dump_html_{typeInfo.id});')
                    else:
                        self.write(f'        dump_html_array<const {typeInfo.baseType}>({param.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{param.name}", 1, dump_html_{typeInfo.id}); // ZRZ')
    ...
    def generate_json_implementation(self, video=False):
        ...
        self.write('\n#include "api_dump_json.h"\n')
        if not video:
            self.write('#include "api_dump_video_json.h"')
...
{{
    if(settings.showAddress()) {{
        ...
        for struct in [ x for x in self.vk.structs.values() if not x.union ]:
            ...
            for member in struct.members:
                ...
                if typeInfo.pointerLevels == 0:
                    if member.name != 'pNext':
                        if member.name == 'apiVersion':
                            self.write(f'    dump_json_value<const {typeInfo.baseType}>(object.{member.name}, NULL, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, OutputApiVersionJSON);')
                        else:
                            self.write(f'    dump_json_value<const {typeInfo.baseType}>(object.{member.name}, NULL, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id});')
                    else:
                        self.write('    if(object.pNext != nullptr){')
                        self.write(f'        dump_json_pNext_trampoline(object.{member.name}, settings, indents + 1);')
                        self.write('    } else {')
                        self.write(f'        dump_json_value<const {typeInfo.baseType}>(object.{member.name}, object.{member.name}, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id});')
                        self.write('    }')

                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'    dump_json_pointer<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id});')
                    else:

                        if not typeInfo.lengthIsMember:
                            self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id}); // IQA')

                        elif typeInfo.lengthIsMember and member.name != 'pCode':
                            if typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id}); // JQA')
                            elif not typeInfo.arrayLength[0].isdigit() or typeInfo.arrayLength[0].isupper():
                                if typeInfo.arrayLength == 'rasterizationSamples':
                                    self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, (object.{typeInfo.arrayLength} + 31) / 32, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id}); // JQA')
                                else:
                                    if typeInfo.maxArrayLength is not None:
                                        self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, std::min(object.{typeInfo.arrayLength}, {typeInfo.maxArrayLength}), settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id}); // JQA')
                                    else:
                                        self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, object.{typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 1, dump_json_{typeInfo.id}); // JQA')
                self.write('')
                if struct.name == 'VkShaderModuleCreateInfo':
                    if member.name == 'pCode':
                        self.write('    if(settings.showShader())')
        ...
        for union in [ x for x in self.vk.structs.values() if x.union ]:
            ...
            for member in union.members:
                ...
                if typeInfo.pointerLevels == 0:
                    self.write(f'    dump_json_value<const {typeInfo.baseType}>(object.{member.name}, NULL, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 2, dump_json_{typeInfo.id});')

                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'    dump_json_pointer<const {typeInfo.baseType}>(object.{member.name}, settings, "{typeInfo.type}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 2, dump_json_{typeInfo.id});')
                    else:
                        self.write(f'    dump_json_array<const {typeInfo.baseType}>(object.{member.name}, {member.length}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{member.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, indents + 2, dump_json_{typeInfo.id}); // OQA')
        ...
        for command in [x for x in self.vk.commands.values() if x.name not in FUNCTION_IMPLEMENTATION_IGNORE_LIST]:
            ...
            for param in command.params:
                ...
                if typeInfo.pointerLevels == 0:
                    self.write(f'        dump_json_value<const {typeInfo.baseType}>({param.name}, NULL, settings, "{typeInfo.type}", "{param.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, 4, dump_json_{typeInfo.id});')

                elif typeInfo.pointerLevels == 1:
                    if typeInfo.arrayLength is None:
                        self.write(f'        dump_json_pointer<const {typeInfo.baseType}>({param.name}, settings, "{typeInfo.type}", "{param.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, 4, dump_json_{typeInfo.id});')

                    else:
                        self.write(f'        dump_json_array<const {typeInfo.baseType}>({param.name}, {typeInfo.arrayLength}, settings, "{typeInfo.type}", "{typeInfo.childType}", "{param.name}", {typeInfo.isStruct}, {typeInfo.isUnion}, 4, dump_json_{typeInfo.id}); // PQA')
    ...
    def fillin_variable_typeinfo(self, variable : Param | Member, parent : Struct | Command):
        ...
        if bracketMatch is not None:
            ...
            if matchText in self.vk.constants:
                typeInfo.arrayLength = self.vk.constants[matchText]
            else:
